...
def test_create_dt_utc(a, expected):
    assert str(create_dt_utc(a)) == expected

@pytest.mark.parametrize("a, b, expected", [
    pytest.param([2025, 10, 20, 18, 0], 5, "2025-10-20 18:00:00+05:00", id="offset 5"),
])
def test_create_dt_utc_delta(a, b, expected):
    assert str(create_dt_utc_delta(a, b)) == expected

@pytest.mark.parametrize("a, b, expected", [
    pytest.param([2025, 9, 25, 18, 0], 180, "2025-09-25 18:00 [UTC+03:00]", id="offset 180"),
])
def test_create_dt_utc_delta_min(a, b, expected):
    assert str(create_dt_utc_delta_min(a, b)) == expected
